    ...
    if (!currentTranslationCode) {
        ...
        if (translatedQuestionOrGroup != nil) {
            NSDictionary * (^ translatedDictionaryBlock)(NSDictionary *, NSDictionary *) = ^(NSDictionary *originalQuestionDictionary, NSDictionary *translatedQuestionDictionary) {
                ...
                if (mutableQuestionDictionary[@"help_text"] && translatedQuestionDictionary[@"help_text"]) {
                    mutableQuestionDictionary[@"help_text"] = translatedQuestionDictionary[@"help_text"];
                }
...
-(void)setTranslationsArray:(NSArray *)translationsArray forSectionWithUUID:(NSString *)sectionUUID withCurrentLocale:(NSString *)localeString {
    if (sectionUUID == nil) {
        ...
        self.navigationItem.rightBarButtonItems = [self.navigationItem.rightBarButtonItems filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(UIBarButtonItem *barButtonItem, NSDictionary *bindings) {
            return (![barButtonItem.title isEqualToString:@"Language"]);
        }]];
        return;
    }
    _translationsArray = nil;
    ...
    UIBarButtonItem *existingTranslationButton = [[self.navigationItem.rightBarButtonItems filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(UIBarButtonItem *barButtonItem, NSDictionary *bindings) {
        return ([barButtonItem.title isEqualToString:@"Language"]);
    }]] lastObject ];
    
    if ([_translationsArray count] > 0 && !existingTranslationButton) {
